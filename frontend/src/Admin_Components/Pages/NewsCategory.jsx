...
const CategoryTable = () => {
  ...
  const deleteCategory = async ({ id, categoryId, subCategoryId, subSubCategoryId }) => {
    ...
    if (categoryId && subCategoryId && subSubCategoryId) {
      url = `http://localhost:3006/api/news/deletesubsubcategory?categoryId=${categoryId}&subCategoryId=${subCategoryId}&subSubCategoryId=${subSubCategoryId}`;
    } else if (categoryId && subCategoryId) {
      url = `http://localhost:3006/api/news/deletesubcategory?categoryId=${categoryId}&subCategoryId=${subCategoryId}`;
    } else {
      url = `http://localhost:3006/api/news/deletecategory?id=${id}`;
    }
                  ...
                  {row.original.subCategories && row.original.subCategories.map((subcategory, subIndex) => (
                    <React.Fragment key={subIndex}>
                      <tr className="border-b border-gray-300 hover:bg-gray-100 transition duration-150 ">
                        <td></td>
                        <td className="py-2 px-8 flex gap-2 hover:text-blue-500 cursor-pointer" onClick={() => navigate(`/NewsCategory/editNewsCategory/${row.original._id}/${subcategory._id}`)}><BsArrowReturnRight />{subcategory.photo && <img src={`http://localhost:3006/api/logo/download/${subcategory.photo}`} alt={subcategory.alt} className="w-6 h-6" />}<span>{subcategory.category}</span></td>
